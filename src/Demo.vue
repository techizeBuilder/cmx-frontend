...
const dynEvents = ref(
  events.value.filter((item) => {
    const startDateIsAfterMinDate = moment(
      item.startDate
    ).isSameOrAfter(date.value[0], 'day');
    const endDateIsBeforeMaxDate = moment(
      item.endDate
    ).isSameOrBefore(date.value[1], 'day');
...
const diffDate = () => {
  const difData =
    moment(date.value[1]).diff(moment(date.value[0]), 'days') + 1;
  // console.log(difData);
  arrDate.value = [];
...
let eventId = 0;
const dragStart = (day, time) => {
  arrDate.value.some((item) => {
    if (item.event.length > 0) {
      return item.event.some((events) => {
        if (
          moment(events.startDate).format('D') == day &&
          moment.utc(events.startDate).format('H') == time.value
        ) {
          eventId = events.id;
          return true; // Exit the loop once the first match is found
        }
      });
    }
  });

  // console.log(eventId); // The ID of the first matching event, or null if no match is found
  // const element = document.getElementById(index);
  // const id = element.getAttribute("itemTable");
  // eventId = id;
};

const dragEnd = (index, time) => {
  const updateData = arrDate.value.find((item) => {
    return item.event.some((events) => {
      return events.id === eventId;
    });
  });
  const foundItem = updateData.event.find(
    (item) => item.id === eventId
  );

  if (foundItem) {
    const datePart = moment(arrDate.value[index].date).format(
      'YYYY-MM-DD'
    );
    const hourPart = moment(time.value, 'HH').format('HH:mm:ss');
    // console.log(eventId);
    events.value = events.value.filter((item) => {
      return item.id !== eventId;
    });
    events.value.push({
      ...foundItem,
      // id: events.value.length + 1, // Assuming you want to increment the id by 1
      startDate: `${datePart}T${hourPart}.000Z`,
      // .set("hour", Math.floor(time.value)) // Set the hour part
      // .set("minute", (time.value - Math.floor(time.value)) * 60) // Set the minute part
      // .toISOString(),
    });
  }
  ...
  // console.log("dynamic event:", arrDate.value);
};
watch(
  eventsValues,
  (newEventsValues) => {
    ...
    dynEvents.value = events.value.filter(
      (item) =>
        eventsValues.value.find(
          (event) => event.label === item.category
        )?.checked
    );
...
watch(date, (newDate) => {
  ...
  dynEvents.value = events.value.filter((item) => {
    const startDateIsAfterMinDate = moment(
      item.startDate
    ).isSameOrAfter(newDate[0], 'day');
    const endDateIsBeforeMaxDate = moment(
      item.endDate
    ).isSameOrBefore(newDate[1], 'day');
    return startDateIsAfterMinDate && endDateIsBeforeMaxDate;
  });
});
watch(selectedDuration, (newSelectedDuration) => {
  eventNo = 0;
  const newEndDate = moment(date.value[0]).add(
    newSelectedDuration,
    'weeks'
  );
  date.value = [
    ...
    <div class="col-sm-3">
      ...
      <div class="card mt-2">
        <div
          class="card-body"
          :style="{
            boxShadow: '0px 3.297px 3.297px 0px rgba(0, 0, 0, 0.25)',
          }"
        >
      ...
      <div
        class="mt-5 d-flex align-items-center justify-content-start gap-2"
      >
      ...
      <div class="mt-3">
        ...
        >
          <input
            type="checkbox"
            v-model="eventsValues[index].checked"
          />
              ...
              >
                ...
                <template v-for="(item, index) in arrDate">
                  ...
                  >
                    <div
                      ...
                      v-if="
                        ...
                        0
                      "
                      v-for="item in item?.event[arrRef[index] - 1]
                        .startMinute / 15"
                    ></div>
                    <div
                      ...
                      draggable="true"
                      @dragover.prevent
                      @drag.prevent="dragStart(index, time)"
                    >
                      <div
                        v-if="
                          !item?.event[arrRef[index] - 1]?.secondCheck
                        "
                      >
                        <div>
                          <img
                  ...
                  <td
                    ...
                    class="emptyTb p-0"
                    @dragover.prevent
                    @drop.prevent="dragEnd(index, time)"
                  >
                    <div
                      class="w-100 h-25"
                      @click="handleModel(time, index, 0)"
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body px-5">
            ...
            <div class="col-sm-12 col-md-6 px-4">
              <div class="d-flex flex-column gap-4">
                <div>
                  ...
                  <div
                    class="customerXBox d-flex align-items-center gap-2"
                  >
                    <img
                      :src="Uplaod"
                      alt="Uplaod"
          ...
          >
            <div
              class="d-flex align-items-end justify-content-end mb-4 gap-4"
            >
              <router-link
                to="/officex"
                class="btn btn-light text-nowrap"
