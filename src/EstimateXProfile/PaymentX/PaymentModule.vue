...
const handleSplitModel = (item) => {
  ...
  const foundPayments = storeEstimateDetails.value?.paymentAmount.find(
    (val) => {
      return val.key === item;
    }
  )?.payments;
  if (foundPayments && foundPayments.length > 0) {
    ...
    foundPayments.forEach((item) => {
      ...
      paymetsObj.DocumentIds = item.DocumentIds;
...
const handleDocumentModel = async (index) => {
  ...
  store.dispatch("setDocumentCategory", "Posted Payment");
                      ...
                      <tr v-for="(item, index) in splitFieldNo">
                        ...
                        <td>
                          ...
                          >
                            ...
                            <option value="Other"
                              >Other (configurable options as needed)</option
                            >
                          </select>
                        </td>
                        <td>
