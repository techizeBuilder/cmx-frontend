...
watch(storeEstimateDetails, () => {
  ...
  PayItem.value.map((item) => {
    switch (item.name) {
      case "Customer":
        item.price = !hasDecimalExceeded(cusSum)
          ? cusSum.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
          : cusSum;
        break;
      case "Insurance":
        item.price = !hasDecimalExceeded(insSum)
          ? insSum.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
          : insSum;
        break;
      case "Grand Total":
        item.price = !hasDecimalExceeded(cusSum + insSum)
          ? (cusSum + insSum).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
          : (cusSum + insSum).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        break;
      default:
        item.price = !hasDecimalExceeded(sum)
