...
import DocumentUploadModule from "./DocumentUploadModule.vue";
...
const handleDocDelete = async () => {
  ...
  try {
    ...
    showSuccessToast(response.data.msg);
    document.getElementById("closeDocDeleteModel").click();
...
const downloadDocument = (item) => {
  fetch(`${apiUrl}/documents/${item.FileRefName}`)
    ...
    .then((blob) => {
      ...
      link.download = item.DocumentName || "default_filename.pdf";
      link.click();
...
<template>
            ...
            >
              <div class="d-flex gap-3">
                <button
                  ...
                  data-bs-target="#DocumentUploadModule"
                  :disabled="docEdit"
                >
  ...
  <div
    ...
    id="docDelete"
    tabindex="-1"
    aria-labelledby="docDeleteLabel"
    aria-hidden="true"
  >
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content w-75">
        <div class="modal-header bg-danger text-white">
          <h5 class="modal-title" id="docDeleteLabel">
            Delete File Notice
          </h5>
          <button
            type="button"
