<script setup>
import ShopusrIcon from "./Icons/ShopusrIcon.svg";
...
const fetchData = async (item) => {
  try {
    ...
    const response = await axios.get(
      `${apiUrl}/estimateChatLink/getChatLink/${item}`,
      config
    );
    chatData.value = response.data.data;
    customerData.value = response.data.customerData;
    estimateIdRef.value = customerData.value._id;
    emit("customerData", customerData.value);
    store.dispatch("setCustomerId", customerData.value.customerId._id);
    props.socket.emit("shopMessage", {
      recieverId: customerData.value.estimateToken,
    });
  } catch (error) {
    handleError(error);
  }
...
const sendMessage = async (item) => {
  try {
    ...
    // response.data.data.token = item;
    props.socket.emit("shopMessage", { recieverId: estimateToken.value });
  } catch (error) {
    handleError(error);
  }
};
...
const handleSendMessage = async () => {
  if (messageData.value.trim() !== "" && !messageEdit.value) {
    ...
    await sendMessage(estimateToken.value);
    await fetchData(estimateToken.value);
    messageData.value = "";
  }
};

const emit = defineEmits(["unseenMesg", "customerData"]);
onUpdated(() => {
  const chatWindow = document.querySelector(".modal-body");
...
<template>
  <div class="message-area">
    <div class="container">
      <div class="row">
        <div class="col-12" style="height: 176vh;">
          <div class="chat-area">
            <!-- chatbox -->
            <div class="chatbox">
                  ...
                  <div class="modal-body">
                    <div class="msg-body">
                      ...
                      >
                        <h6>
                          {{
                            customerData?.customerId?.lastName?.toUpperCase()
                          }},
                          {{
                            customerData?.customerId?.firstName?.toUpperCase()
                          }}
                        </h6>
                        <h6>
                          {{
                            formatPhoneNumber(customerData?.customerId?.phone)
                          }}
                        </h6>
                        <h6>{{ customerData?.customerId?.email }}</h6>
                      </div>
                      <ul class="px-2 pb-2">
                                  ...
                                  <div class="d-flex justify-content-between">
                                    ...
                                    <span>
                                      <p class="shopUserp m-0 fw-bold">
                                        {{
                                          customerData?.shopId?.shopName?.toUpperCase()
                                        }}
                                      </p>
                                      <img
                                        :src="ShopusrIcon"
