      ...
      <div class="row justify-content-center">
        <div class="col-lg-8">
          <div class="card">
            ...
            <div class="card-body">
              <!-- Loading State -->
              <div v-if="isLoadingShop" class="text-center py-4">
                <div class="spinner-border text-primary" role="status">
                  <span class="visually-hidden">Loading shop data...</span>
                </div>
                <p class="mt-2">Loading shop information...</p>
              </div>
                ...
                <div v-if="currentStep === 2">
                  ...
                  <div class="text-center">
                    <div class="logo-upload-area mb-3" @click="$refs.logoInput.click()">
                      <div v-if="!logoPreview && !currentLogo" class="upload-placeholder">
                      ...
                      <div v-else class="logo-preview">
                        <img :src="logoPreview || currentLogo" alt="Logo Preview" class="img-fluid" />
                      </div>
                    </div>
                    <input 
                      ref="logoInput"
                    ...
                    <div class="mt-2">
                      ...
                      <small class="text-muted d-block mt-2" v-if="!logoPreview && currentLogo">
                        Current logo will be kept if no new logo is uploaded
                      </small>
                    </div>
                  </div>
                </div>
            ...
            <div class="card-footer" v-if="!isLoadingShop">
...
// Check authentication and load shop data on mount
onMounted(() => {
  ...
  if (!shopId) {
    showErrorToast('Shop ID is required')
  ...
  try {
    ...
    if (response && response.success && response.data) {
      ...
      formData.value = {
        ...
        shopPhone: response.data.contact?.phone || '',
        shopEmail: response.data.contact?.email || '',
        shopWebsite: response.data.contact?.website || '',
        ...
        dateFormat: response.data.settings?.dateFormat || '',
        timeFormat: response.data.settings?.timeFormat || '',
        logo: null, // New logo file will be set here
        adminName: response.data.admin?.name || '',
        adminPhone: response.data.admin?.phone || '',
        adminEmail: response.data.admin?.email || ''
      }
      
      // Set current logo for display
      if (response.data.logo) {
        currentLogo.value = getImageUrl(response.data.logo)
      }
...
const submitForm = async () => {
  ...
  try {
    ...
    // Add all fields (all are optional for update)
    ...
    console.log('FormData being sent to API for update:')
    for (let [key, value] of formDataToSubmit.entries()) {
      console.log(`${key}:`, value)
    }
    
    // Call API with shopId from route
    const response = await shopService.updateShop(route.params.shopId, formDataToSubmit)
    
    if (response && response.success) {
      showSuccessToast(response.message || 'Shop updated successfully!')
      router.push('/cmx-super/dashboard')
    } else {
      throw new Error('Invalid response format')
    }
    ...
    if (error.response && error.response.data) {
      ...
      showErrorToast('Failed to update shop. Please try again.')
    }
  } finally {
