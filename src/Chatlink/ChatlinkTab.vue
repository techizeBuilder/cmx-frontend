...
import CMXCustomer from "./Icons/CMXCustomer.png";
import Shopuser from "./Icons/Shopuser.png";
...
const seenMessage = async (item) => {
  try {
    const response = await axios.get(
      `${apiUrl}/estimateChatLink/Update/Seen/${item}/Admin`
    );
    if (response.data.broadCast) {
      socket.emit("customerMessage", {
        recieverId: route.params.token,
      });
    }
  } catch (error) {
    console.log(error);
...
const fetchData = async () => {
  try {
    ...
    const response = await axios.get(
      `${apiUrl}/estimateChatLink/getChatLink/${route.params.token}`,
      config
    );
    chatData.value = response.data.data;
    customerName.value = response.data.customerData.customerId.firstName;
    shopId.value = response.data.customerData.customerId.shopId;
    return {
      customerId: response.data.customerData.customerId._id,
      shopName: response.data.customerData.shopId.shopName,
      estimateId: response.data.customerData._id,
    };
  } catch (error) {
    handleError(error);
  }
...
const sendMessage = async () => {
  try {
    ...
    const data = {
      ...
      estimateToken: route.params.token,
    };
...
const shopNameRef = ref("");
const estimateIdRef = ref("");
onMounted(async () => {
  isLoading.value = true;
  const { customerId, shopName, estimateId } = await fetchData();
  shopNameRef.value = shopName;
  estimateIdRef.value = estimateId;
  await seenMessage(route.params.token);
  socket.on("connect", () => {
    // console.log(socket.id);
  });
  socket.emit("customerConnect", route.params.token);
  socket.on("shopMessage", async (data) => {
    await fetchData();
    await seenMessage(route.params.token);
  });
...
const handleSendMessage = async () => {
  if (messageSend.value.trim() !== "" && !messageEdit.value) {
...
onUpdated(() => {
  const chatWindow = document.getElementById("scroll");
...
<template>
  <loading
    ...
    color="#E6F0FF"
  />
  <h5 class="text-center pt-5 pb-4 fw-bold">Chatlink</h5>
